 ...
 */
export function validate_animation_bones_match (
  ...
  const animation_bone_names = get_animation_bone_names(animation_clips)
 ...
 */
function get_animation_bone_names (animation_clips: AnimationClip[]): Set<string> {
  const bone_names = new Set<string>()
  animation_clips.forEach((clip) => {
    clip.tracks.forEach((track) => {
      const bone_name = extract_bone_name_from_track(track.name)
      if (bone_name !== null) {
 ...
 */
function extract_bone_name_from_track (track_name: string): string | null {
